import * as React from "react";
import styled, { css } from "styled-components";
import classNames from "classnames";

// Components
import Form, { FormCol, FormRow } from "./Form";

// Utils
import { callAll } from "../utils";

const baseStyles = css`
  position: absolute;
  top: 0;
  left: 0;
  bottom: 0;
  right: 0;
  border: var(--default-border);
  padding: var(--default-padding);
  margin: 0;
  width: 100%;
  height: 100%;
  padding-left: inherit;

  /* ======================================
STATE STYLES
====================================== */

  &:focus {
    outline-color: var(--brand-01-10);
  }

  &:invalid {
    outline-color: var(--color-error);
  }
`;

const FieldContainer = styled.div`
  width: 100%;
  position: relative;
  padding: var(--default-padding);
  isolation: isolate;

  /* ======================================
  FORM STYLES
  ====================================== */

  ${Form} & {
    margin-bottom: var(--default-margin);
  }

  ${Form} &,
  ${Form} & > input,
  ${Form} & > select {
    border-radius: var(--default-border-radius);
  }

  ${FormRow} &,
  ${FormRow} & > input,
  ${FormRow} & > select {
    border-radius: 0px;
  }

  ${FormRow} &:first-child,
  ${FormRow} &:first-child > input,
  ${FormRow} &:first-child > select {
    border-top-left-radius: var(--default-border-radius);
    border-bottom-left-radius: var(--default-border-radius);
  }

  ${FormRow} &:last-child,
  ${FormRow} &:last-child > input,
  ${FormRow} &:last-child > select {
    border-top-right-radius: var(--default-border-radius);
    border-bottom-right-radius: var(--default-border-radius);
  }

  ${FormRow} &:not(:last-child),
  ${FormRow} &:not(:last-child) > input:not(:focus),
  ${FormRow} &:not(:last-child) > select:not(:focus) {
    border-right: none;
  }

  ${FormCol} &,
  ${FormCol} & > input,
  ${FormCol} & > select {
    border-radius: 0px;
  }

  ${FormCol} &:first-child,
  ${FormCol} &:first-child > input,
  ${FormCol} &:first-child > select {
    border-top-left-radius: var(--default-border-radius);
    border-top-right-radius: var(--default-border-radius);
  }

  ${FormCol} &:last-child,
  ${FormCol} &:last-child > input,
  ${FormCol} &:last-child > select {
    border-bottom-left-radius: var(--default-border-radius);
    border-bottom-right-radius: var(--default-border-radius);
  }

  ${FormCol} &:not(:last-child),
  ${FormCol} &:not(:last-child) > input,
  ${FormCol} &:not(:last-child) > select {
    border-bottom: none;
    margin-bottom: 0;
  }

  ${FormCol} ${FormRow} &,
  ${FormCol} ${FormRow} & > input,
  ${FormCol} ${FormRow} & > select {
    border-radius: 0px;
    margin-bottom: 0;
  }

  ${FormCol} ${FormRow}:first-child &,
  ${FormCol} ${FormRow}:first-child & > input,
  ${FormCol} ${FormRow}:first-child & > select {
    border-top-left-radius: 0px;
    border-top-right-radius: 0px;
  }

  ${FormCol} ${FormRow}:first-child &:first-child,
  ${FormCol} ${FormRow}:first-child &:first-child > input,
  ${FormCol} ${FormRow}:first-child &:first-child > select {
    border-top-left-radius: var(--default-border-radius);
  }

  ${FormCol} ${FormRow}:first-child &:last-child,
  ${FormCol} ${FormRow}:first-child &:last-child > input,
  ${FormCol} ${FormRow}:first-child &:last-child > select {
    border-top-right-radius: var(--default-border-radius);
  }

  ${FormCol} ${FormRow}:last-child &,
  ${FormCol} ${FormRow}:last-child & > input,
  ${FormCol} ${FormRow}:last-child & > select {
    border-bottom-left-radius: 0px;
    border-bottom-right-radius: 0px;
    margin-bottom: var(--default-margin);
  }

  ${FormCol} ${FormRow}:last-child &:not(:last-child),
  ${FormCol} ${FormRow}:last-child &:not(:last-child) > input,
  ${FormCol} ${FormRow}:last-child &:not(:last-child) > select {
    margin-bottom: var(--default-margin);
  }

  ${FormCol} ${FormRow}:last-child &:not(:last-child) > input,
  ${FormCol} ${FormRow}:last-child &:not(:last-child) > select {
    border-bottom: var(--default-border);
  }

  ${FormCol} ${FormRow}:last-child &:first-child,
  ${FormCol} ${FormRow}:last-child &:first-child > input,
  ${FormCol} ${FormRow}:last-child &:first-child > select {
    border-bottom-left-radius: var(--default-border-radius);
  }

  ${FormCol} ${FormRow}:last-child &:last-child,
  ${FormCol} ${FormRow}:last-child &:last-child > input,
  ${FormCol} ${FormRow}:last-child &:last-child > select {
    border-bottom-right-radius: var(--default-border-radius);
  }

  ${FormCol} ${FormRow}:not(:last-child) &,
  ${FormCol} ${FormRow}:not(:last-child) & > input,
  ${FormCol} ${FormRow}:not(:last-child) & > select {
    border-bottom: none;
  }
`;

const BaseInput = styled.input`
  ${baseStyles}

  /* ======================================
  DATEPICKER STYLES
  ====================================== */

  /*
   * Handle styles for date input when not populated/focused
   * (Chrome only)
   */
  &::-webkit-datetime-edit-text,
  &::-webkit-datetime-edit-month-field,
  &::-webkit-datetime-edit-day-field,
  &::-webkit-datetime-edit-year-field {
    color: transparent;
    transition: color 0.2s cubic-bezier(0, 0, 0.2, 1);
  }

  /*
   * Handle styles for date input when populated/focused
   * (Chrome only)
   */
  &:focus::-webkit-datetime-edit-text,
  &:focus::-webkit-datetime-edit-month-field,
  &:focus::-webkit-datetime-edit-day-field,
  &:focus::-webkit-datetime-edit-year-field,
  &.hold-label::-webkit-datetime-edit-text,
  &.hold-label::-webkit-datetime-edit-month-field,
  &.hold-label::-webkit-datetime-edit-day-field,
  &.hold-label::-webkit-datetime-edit-year-field {
    color: var(--color-text-muted);
  }

  /*
   * Because the padding set on the input
   * pushes the content down, we need to
   * compensate against this for the
   * datepicker icon
   */
  &::-webkit-calendar-picker-indicator {
    margin-bottom: 1rem;
  }
`;

const Select = styled.select`
  ${baseStyles}

  /*
   * Removing the default arrow
   */
  -webkit-appearance: none;
  appearance: none;

  /*
   * Custom arrow
   */
  background-image: url("/caret-down-light.svg");
  background-size: 1rem;
  background-repeat: no-repeat;
  background-position: calc(100% - var(--default-padding)) center;
`;

const InputWithLabel = styled(BaseInput)`
  padding: var(--default-padding) var(--default-padding) 0 0;
  padding-left: inherit;
`;

const Label = styled.label`
  position: relative;
  display: inline-block;
  z-index: 1;
  pointer-events: none;
  transform: none;
  color: var(--color-text-muted);
  background-color: inherit;
  font-size: var(--font-size-xxs);
  transform: scale(1.3);
  transform-origin: bottom left;
  transition: transform 0.2s cubic-bezier(0, 0, 0.2, 1);

  ${InputWithLabel}:focus + &,
  ${InputWithLabel}.hold-label + &,
  ${InputWithLabel}:autofill + &,
  ${InputWithLabel}:-webkit-autofill + &
  /* Date input will start with a value */ {
    color: var(--brand-01-100);
    transform: translate(0px, -0.7rem) scale(1);
  }
`;

type InputProps = React.InputHTMLAttributes<HTMLInputElement> & {
  label?: string;
};

export const Input: React.FC<InputProps> = ({
  label,
  id,
  onBlur,
  onChange,
  ...props
}) => {
  // Ensure the label points to the input with the same id.
  if (label && !id) {
    throw new Error("Please provide an id for the TextField");
  }
  const [shouldHoldLabel, setShouldHoldLabel] = React.useState(false);
  const classes = classNames(props.className, {
    // On blur, if there is user input then we want to keep the label hovering.
    "hold-label": shouldHoldLabel,
  });

  const Input = label ? InputWithLabel : BaseInput;

  const handleBlur: React.EventHandler<React.FocusEvent<HTMLInputElement>> = (
    e
  ) => {
    // On blur, if there is user input then we want to keep the label hovering.
    if (e.target.value.length >= 1) {
      setShouldHoldLabel(true);
    } else {
      setShouldHoldLabel(false);
    }
  };

  const handleChange: React.EventHandler<
    React.ChangeEvent<HTMLInputElement>
  > = (e) => {
    if (e.target.value.length >= 1) {
      setShouldHoldLabel(true);
    } else {
      setShouldHoldLabel(false);
    }
  };

  return (
    <FieldContainer>
      <Input
        id={id}
        className={classes}
        onBlur={callAll<React.FocusEvent<HTMLInputElement>>(handleBlur, onBlur)}
        onChange={callAll(handleChange, onChange)}
        {...props}
      />
      <Label htmlFor={id}>{label}</Label>
    </FieldContainer>
  );
};

type DropdownProps = React.SelectHTMLAttributes<HTMLSelectElement> & {
  options: string[];
};

export const Dropdown: React.FC<DropdownProps> = ({ options, ...props }) => {
  return (
    <FieldContainer>
      <Select {...props}>
        {options.map((option) => (
          <option key={option} value={option}>
            {option}
          </option>
        ))}
      </Select>
    </FieldContainer>
  );
};
